...
class dstat_plugin(dstat):
    ...
    def extract(self):
        ...
        self.pidset4 = {}
        self.outputs = []
        self.set2 = {'time':[], 'pid':[], 'name':[], 'cpu':[], 'mem':[], 'io_read':[], 'io_write':[]}
        ...
        for pid in proc_pidlist():
            ...
            ucpu = (self.pidset2[pid] - self.pidset1[pid]) * 1.0 / elapsed / cpunr
            name = l[1][1:-1]
            self.set2['cpu'].append(unicode(ucpu))
            self.set2['pid'].append(pid)
            self.set2['name'].append(getnamebypid(pid, name))

            # 获取mem数据
            if len(l) < 23: continue
            umem = int(l[23]) * pagesize
            self.set2['mem'].append(unicode(umem))
            ...
            self.set2['io_read'].append(unicode(read_usage))
            self.set2['io_write'].append(unicode(write_usage))
            self.set2['time'].append(time.strftime('%H:%M:%S', time.localtime()))

        for key in self.set2:
            self.val[key] = '/n'.join(self.set2[key])
